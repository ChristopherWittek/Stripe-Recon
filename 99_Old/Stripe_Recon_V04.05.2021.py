...
def floatCleanser(dirty_series) :
    try :
        dirty_series = dirty_series.str.replace(',','',regex=True).str.replace('€','',regex=True).str.replace('$','',regex=True).str.replace('£','',regex=True).str.replace('ý','',regex=True).str.replace(r'â\x82','',regex=True).str.replace('Â','',regex=True).str.rstrip(')').str.rstrip(' ').str.lstrip('ý').str.replace('Can','',regex=True).str.replace('--','-',regex=True).str.replace('¬','',regex=True).str.replace('(','-',regex=True).str.replace('ý','',regex=True).str.replace('-ï¿œï¿œï¿','',regex=True).str.replace('(ï¿œï¿œï','-',regex=True).str.replace('(ï¿œï¿œï¿','-',regex=True).str.replace(' ','',regex=True).astype(float)
...
def floatCleanser(dirty_series) :
    dirty_series = dirty_series.str.replace(',','',regex=True).str.replace('€','',regex=True).str.replace('$','',regex=True).str.replace('£','',regex=True).str.replace(r'â\x82','',regex=True).str.replace('Â','',regex=True).str.rstrip(')').str.replace('Can','',regex=True).str.replace('--','-',regex=True).str.replace('¬','',regex=True).str.replace('(','-',regex=True).str.replace('ý','',regex=True).str.replace('-ï¿œï¿œï¿','',regex=True).str.replace('-ï¿œï¿œï','').str.replace('-ï¿œï¿œ','').str.replace(r'',0.00,regex=True)
    try :
        dirty_series = dirty_series.astype(float)
    except Exception as e :
        print(e)
    global clean_series
    clean_series = dirty_series
...
journal_entry['Sales Channel'] = journal_entry['Sales Channel'].fillna('')#.replace(np.nan,'Corporate')#,inplace=True
journal_entry['Region'] = journal_entry['Region'].fillna('EMEA : Western Europe')#,inplace=True
